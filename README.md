...
- None outstanding. Previous defects verified fixed and `test.fail()`
  markers removed: `serviceLocation` raw key in New Booking, modal
  `role="dialog"` semantics, branding `htmlFor` labels, bookings + routes
  internal table scroll at 1440px, driver stop-row nested interactives
  (now a real button + sibling link).
